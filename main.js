...
// HÀM TÍNH THỜI GIAN "xxx TRƯỚC" SIÊU NHẸ (Chỉ chạy 1 lần khi mở/render)
...
document.addEventListener('mouseover', (e) => { if(e.target.closest('button, .photo-card, input, .lucide, .close-modal, .user-profile, .noti-item, .notification-wrapper, .upload-preview-area, #detailImageContainer, .friend-item, .comment-action-btn')) document.body.classList.add('hovering'); });
document.addEventListener('mouseout', (e) => { if(e.target.closest('button, .photo-card, input, .lucide, .close-modal, .user-profile, .noti-item, .notification-wrapper, .upload-preview-area, #detailImageContainer, .friend-item, .comment-action-btn')) document.body.classList.remove('hovering'); });
...
document.getElementById("chatBtn").onclick = () => { chatM.classList.add("active"); loadFriendList(); };
async function loadFriendList() {
    const listEl = document.getElementById("friendList"); const s = await getDocs(query(collection(db, "friends_chat"), where("users", "array-contains", currentUser.uid)));
    if (s.empty) { listEl.innerHTML = "<p style='padding:20px; color:var(--text-muted); text-align:center;'>Chưa có bạn bè nào.</p>"; return; }
    let html = ""; s.forEach(d => { 
        const data = d.data(); const friendId = data.users.find(id => id !== currentUser.uid); const friendName = data.userNames[friendId]; 
        let badge = '';
        ...
        html += `<div class="friend-item" onclick="openChatRoom('${d.id}', '${friendName}')"><div class="friend-avatar">${friendName.charAt(0).toUpperCase()}</div><span class="friend-name">${friendName}</span>${badge}</div>`; 
    });
...
window.openChatRoom = (chatId, friendName) => {
    ...
    chatRoomUnsub = onSnapshot(doc(db, "friends_chat", chatId), (docSnap) => {
        ...
        if (room.status === 'pending') {
            if (room.requesterId === currentUser.uid) { headerArea.innerHTML = `Đang chat với: ${friendName} <span style="font-size:0.8rem;color:#f59e0b;font-weight:normal;">(Chờ chấp nhận)</span>`; } 
                ...
                document.getElementById("acceptChatBtn").onclick = async () => { await updateDoc(doc(db, "friends_chat", chatId), { status: 'accepted' }); showToast("Đã trở thành bạn bè!"); loadFriendList(); };
            }
    ...
    chatMsgUnsub = onSnapshot(query(collection(db, "friends_chat", chatId, "messages"), orderBy("timestamp", "asc")), (snap) => {
        ...
        snap.forEach(d => { 
            ...
            const timeStr = timeAgo(msg.timestamp); // GỌI HÀM TIME AGO Ở ĐÂY
...
document.getElementById("chatSendBtn").onclick = async () => { const text = document.getElementById("chatInput").value.trim(); if(!text || !currentActiveChatId) return; document.getElementById("chatInput").value = ""; await addDoc(collection(db, "friends_chat", currentActiveChatId, "messages"), { senderId: currentUser.uid, text: text, timestamp: serverTimestamp() }); };
...
function listenChatNotifications() {
    ...
    chatNotiUnsub = onSnapshot(query(collection(db, "friends_chat"), where("users", "array-contains", currentUser.uid)), (snap) => {
        let pendingReqCount = 0; snap.forEach(d => { const data = d.data(); if (data.status === 'pending' && data.requesterId !== currentUser.uid) { pendingReqCount++; } });
        const chatBadge = document.getElementById("chatBadge");
        if(chatBadge) { if(pendingReqCount > 0) { chatBadge.innerText = pendingReqCount; chatBadge.style.display = "flex"; } else { chatBadge.style.display = "none"; } }
    });
}
...
document.getElementById("loginSubmitBtn").onclick = async () => { try { const res = await signInWithEmailAndPassword(auth, document.getElementById("emailInput").value, document.getElementById("passwordInput").value); await logSecurityAction(res.user.uid, "LOGIN", `Đăng nhập hệ thống`); showToast("Đăng nhập thành công!"); } catch(e) { showToast("Sai thông tin!", "error"); } };
...
document.getElementById("postsGrid").addEventListener('click', async (e) => {
    ...
    if (heartBtn) {
        ...
        if (icon) {
            if (icon.classList.contains('icon-filled')) { updateDoc(pRef, { likedBy: arrayRemove(currentUser.uid) }); icon.classList.remove('icon-filled'); if(pData.userId !== currentUser.uid) deleteDoc(doc(db, "notifications", `like_${heartBtn.dataset.id}_${currentUser.uid}`)); } 
            else { updateDoc(pRef, { likedBy: arrayUnion(currentUser.uid) }); icon.classList.add('icon-filled'); if(pData.userId !== currentUser.uid) setDoc(doc(db, "notifications", `like_${heartBtn.dataset.id}_${currentUser.uid}`), { userId: pData.userId, fromUserId: currentUser.uid, fromUserName: currentUser.email.split('@')[0], type: 'like', postId: heartBtn.dataset.id, postImage: pData.image_url, timestamp: serverTimestamp(), isRead: false }); }
        } return;
    }
            ...
            if (chatDoc.exists()) { 
                ...
                if (data.status === 'accepted') { addFriendBtn.innerHTML = `<i data-lucide="check" style="width:14px;"></i> Bạn bè`; addFriendBtn.onclick = null; }
                    ...
                    if (data.requesterId === currentUser.uid) { addFriendBtn.innerHTML = `<i data-lucide="clock" style="width:14px;"></i> Đã gửi yêu cầu`; addFriendBtn.onclick = null; }
                        ...
                        addFriendBtn.onclick = async () => { await updateDoc(doc(db, "friends_chat", chatId), { status: 'accepted' }); addFriendBtn.innerHTML = `<i data-lucide="check" style="width:14px;"></i> Bạn bè`; showToast("Đã kết bạn! Mở thanh Chat để nhắn tin."); await logSecurityAction(currentUser.uid, "ADD_FRIEND", `Kết bạn với ${currentPostData.userName}`); };
                    }
                }
            } else {
                addFriendBtn.innerHTML = `<i data-lucide="user-plus" style="width:14px;"></i> Kết bạn`;
                addFriendBtn.onclick = async () => { 
                    await setDoc(doc(db, "friends_chat", chatId), { users: [currentUser.uid, currentPostData.userId], userNames: { [currentUser.uid]: currentUser.email.split('@')[0], [currentPostData.userId]: currentPostData.userName }, status: 'pending', requesterId: currentUser.uid, createdAt: serverTimestamp() }); 
...
// RENDER BÌNH LUẬN KÈM TÍNH THỜI GIAN "xxx TRƯỚC"
let unsub = null;
function loadDetailComments(pId) {
    ...
    unsub = onSnapshot(query(collection(db, "football_posts", pId, "comments"), orderBy("timestamp", "asc")), (snap) => {
        ...
        Object.values(commentsMap).filter(c => !c.replyTo).forEach(c => {
            const timeStr = timeAgo(c.timestamp); // GỌI HÀM TIME AGO Ở ĐÂY
            ...
            Object.values(commentsMap).filter(r => r.replyTo === c.id).forEach(r => {
                const rTimeStr = timeAgo(r.timestamp); // GỌI HÀM TIME AGO CHO REPLY
...
document.getElementById("detailCommentList").addEventListener('click', async (e) => {
    ...
    if (delBtn) { showConfirm("Xóa bình luận này?", async () => { await deleteDoc(doc(db, "football_posts", currentInteractPostId, "comments", delBtn.dataset.id)); showToast("Đã xóa!"); }); return; }
